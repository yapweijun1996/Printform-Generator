    ...
    <div className="relative w-full h-full bg-[#1e1e1e] flex flex-col group overflow-hidden">
      ...
      <div className="flex-none h-6 bg-[#007acc] text-white flex items-center px-3 justify-between text-[10px] select-none z-10">
        <div className="flex gap-3">
             <span className="flex items-center gap-1"><span className="font-bold">Ln</span> {code.split('\n').length}, <span className="font-bold">Col</span> 1</span>
        </div>
        <div className="flex gap-3">
            <span>HTML</span>
            <span>UTF-8</span>
            <span className="hover:bg-white/20 px-1 rounded cursor-pointer transition-colors" title="Prettier">Prettier</span>
        </div>
      </div>
    </div>
  );
