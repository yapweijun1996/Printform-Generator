...
- **Agentic RAG (PrintForm.js SOP Expert)**: Automatically retrieves relevant rules from `docs/PRINTFORM_JS_SOP.md` and injects them into the prompt, so generated HTML stays PrintForm.js-compliant.
- **Auto Compliance Hints**: Runs a best-effort local print-safe check and surfaces issues for the model to fix first (ERROR → WARN).
...
### Where the SOP RAG lives

- SOP loader/retriever: `services/printformSop/retrieveSopInfo.ts`
- Prompt injection (per turn): `services/geminiService.ts`
