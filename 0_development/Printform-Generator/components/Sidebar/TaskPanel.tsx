        ...
        {tasks.length === 0 ? (
          ...
          <div className="space-y-3">
             {tasks.map((task, index) => (
                 <div key={task.id} className={`flex items-start gap-3 p-3 border rounded-lg shadow-sm transition-all duration-300 ${
                     task.status === 'in_progress' ? 'bg-blue-50 border-blue-200 ring-1 ring-blue-100' : 
                     task.status === 'failed' ? 'bg-red-50 border-red-200' : 'bg-white border-erp-100'
                 }`}>
                    ...
                    <div className="flex-1">
                        <div className={`text-sm ${
                            task.status === 'completed' ? 'text-erp-400 line-through' : 
                            task.status === 'failed' ? 'text-red-800' : 'text-erp-800 font-medium'
                        }`}>
                            {task.description}
                        </div>
                        <div className={`text-[10px] mt-1 uppercase font-bold tracking-wide flex items-center gap-1 ${
                             task.status === 'in_progress' ? 'text-blue-600' :
                             task.status === 'failed' ? 'text-red-500' : 'text-erp-400'
                        }`}>
                            {task.status.replace('_', ' ')}
                        </div>
                    </div>
                    <div className="text-xs font-mono text-erp-300">
                        #{index + 1}
                    </div>
                 </div>
             ))}
